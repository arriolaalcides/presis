...
de.observaciones, 
de.flete, 
de.costo_por_contrareembolso, 
de.seguro, 
de.costo_src, 
de.costo_despacho_a_expreso, 
de.custodia,
de.monto_guia_web, 
de.costo_por_monitoreo_activo, 
de.costo_adicional1, 
de.costo_adicional2, 
de.costo_adicional3, 
de.totalFlete, 
...
if ($cliente != "") {
    $bandera ? $sql = $sql . "AND upper(r.cliente_id) = upper('" . $cliente . "') " : $sql = $sql . "upper(r.cliente_id) = upper('" . $cliente . "') ";
    $bandera = true;
}
if ($nroCta != "") {
...
if ($facturado == "0") {
    $bandera ? $sql = $sql . "AND de.nro_factura IS NULL " : $sql = $sql . "de.nro_factura IS NULL ";
    $bandera = true;
}
if ($facturado == "1") {
    $bandera ? $sql = $sql . "AND de.nro_factura IS NOT NULL " : $sql = $sql . "de.nro_factura IS NOT NULL ";
    $bandera = true;
}
...
if($bandera!=1){
    if($estado != "null"){
        ...
        for($i=0; $i<count($estado); $i++){
            if($i==0){
                $bandera ? $sql = $sql . " upper(r.estado_id) = upper('" . $estado[$i] . "') " : $sql = $sql . " upper(r.estado_id) = upper('" . $estado[$i] . "') ";
                $bandera = true;
            }else{
...
if($sinEstado=="true"){
	...
	if($bandera){
    		$sql = $sql . "AND estado_id != 106";
	}else{
    		$sql = $sql . "estado_id != 106";
	}
}
...
echo '<tr>';
    ...
    echo '<td>CPC</td>';
    echo '<td>Seguro</td>';
    echo '<td>Costo SRC</td>';
    echo '<td>Costo de Expreso</td>';
    echo '<td>Custodia</td>';
    echo '<td>$ Guia WEB</td>';
    echo '<td>C. M. Activo</td>';
    echo '<td>Costo A 1</td>';
    echo '<td>Costo A 2</td>';
    echo '<td>Costo A 3</td>';
    echo '<td>Total Flete</td>';
    echo '<td>Fecha Pactada</td>';
    ...
    echo '<td>Obs 4</td>';
    echo '<td>Zona</td>';
    echo '<td>Obs Estado</td>';
    echo '<td>Ceco</td>';
    echo '<td>Usuario</td>';
    echo '<td>Fecha Confirmada</td>';
echo '</tr>';
while($row = mysql_fetch_array($result)){
    echo '<tr>';
        ...
        //echo '<td>'.$row['peso'].'</td>';
    ?>
    <td style="mso-number-format:'0\.00';"><?php echo $row['peso']; ?></td>
    <?php
        echo '<td>'.$row['alto'].'</td>';
        echo '<td>'.$row['largo'].'</td>';
        echo '<td>'.$row['ancho'].'</td>';
        ?>
        <td style="mso-number-format:'0.00';"><?php echo $row['valor_declarado']; ?></td>
        <?php
    ?>
    <td style="mso-number-format:'0.00';"><?php echo $row['contrareembolso']; ?></td>
    <?php
        ...
        echo '<td>'.$row['fechaplanilla'].'</td>';
        echo '<td>'.$row['observaciones'].'</td>';
    ?>
    <td style="mso-number-format:'0.00';"><?php echo $row['flete']; ?></td>
    <?php
        //echo '<td>'.$row['flete'].'</td>';
        echo '<td>'.$row['costo_por_contrareembolso'].'</td>';
    ?>
    <td style="mso-number-format:'0.00';"><?php echo $row['seguro']; ?></td>
    <?php
        //echo '<td>'.$row['seguro'].'</td>';
        echo '<td>'.$row['costo_src'].'</td>';
        echo '<td>'.$row['costo_despacho_a_expreso'].'</td>';
        echo '<td>'.$row['custodia'].'</td>';
        echo '<td>'.$row['monto_guia_web'].'</td>';
        echo '<td>'.$row['costo_por_monitoreo_activo'].'</td>';
        echo '<td>'.$row['costo_adicional1'].'</td>';
        echo '<td>'.$row['costo_adicional2'].'</td>';
        echo '<td>'.$row['costo_adicional3'].'</td>';
        echo '<td>'.$row['totalFlete'].'</td>';
        ...
        echo '<td>'.$row['comHorario'].' hs</td>';
        echo '<td>'.$row['documento'].'</td>';
        echo '<td>'.$row['obs1'].'</td>';
        echo '<td>'.$row['obs2'].'</td>';
        echo '<td>'.$row['obs3'].'</td>';
        echo '<td>'.$row['obs4'].'</td>';
        echo '<td>'.$row['comZona'].'</td>';
        echo '<td>'.$row['obsEstado'].'</td>';
        echo '<td>'.$row['cecoDesc'].'</td>';
