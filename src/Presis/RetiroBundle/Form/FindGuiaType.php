...
class FindGuiaType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
            ...
            ->add('fechaUdesde', 'text', array('required' => false, 'label'=> 'F. U. Estado Desde', 'mapped'=>false))
            ->add('fechaUhasta', 'text', array('required' => false, 'label'=> 'F. U. Estado Hasta', 'mapped'=>false))

            ->add('fechaUplanillaDesde', 'text', array('required' => false, 'label'=> 'F. U. planilla Desde', 'mapped'=>false))
            ->add('fechaUplanillaHasta', 'text', array('required' => false, 'label'=> 'F. U. planilla Hasta', 'mapped'=>false))

            ->add('fechaFacturaDesde', 'text', array('required' => false, 'label'=> 'F. Fac. Desde', 'mapped'=>false))
            ->add('fechaFacturaHasta', 'text', array('required' => false, 'label'=> 'F. Fac. Hasta', 'mapped'=>false))
...
//                    'choices' => $user->getCliente()->getRemitente(),
        ...
        if($user->hasRole('ROLE_CLIENTE')){
            $builder->add('estado', 'entity', array(
                ...
                'required' => false,
                'mapped' => false,
                'attr' => array('style' => 'width:250px; height:270px'),
                'query_builder' => function(EntityRepository $er) {
                    return $er->createQueryBuilder('e')
                        ->where('e.id != 106')
                        ->andWhere('e.seleccionableParaWeb != FALSE');
                },
            ));
        }else{
            $builder->add('estado', 'entity', array(
                ...
                $builder->add('sinEstado', 'checkbox', array(
                    'required' => false,
                    'mapped' => false,));
        }
            /*->add('sinEstado', 'choice', array(
                'label' => "Sin estado",
            ...
            /*->add('sinFactura', 'checkbox', array(
                'required' => false,
                'mapped' => false,))*/
        $builder->add('datosenvios', new \Presis\DatosEnviosBundle\Form\DatosEnviosType($this->securityContext, "buscar", $this->empresa));
    }
