...
class DatosEnviosController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            //DEIVID Agregado no guardaba servicio en retiro 
            $entity->setServicio($entity->getDatosEnvios()->getServicio());
            $em->persist($entity);
            $em->flush();
            ...
            if(!$user->hasRole('ROLE_ADMIN')){
                if($user->getCliente()->getEmpresa()=='MOVISTAR'){
                    ...
                    if($form['datosenvios']['servicio']->getData()=='AMBA - Express' &&  $form['gestioncel']['tiposervicio']->getData()=='Vip' && $form['gestioncel']['tipocliente']->getData()=='CORPORATIVO'){
                    /*    $mensaje = 'Se envio un mail';
                        $envio = false;
                         $from1 = new SendGrid\Email("Shipro", "deivid921015@gmail.com");
                        $subject1 = "Confirmacion Ticket  ";
                        $to1 = new SendGrid\Email("Confirmando", "deivid921015@gmail.com");
                        $content1 = new SendGrid\Content("text/html", "Estimado/a: <b>DEIVID VIP EXPRESS</b>");
                        $mail1 = new SendGrid\Mail($from1, $subject1, $to1, $content1);
                        //$apiKey = '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';
                        $apiKey = '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';
                        $sg1 = new \SendGrid($apiKey);
                        $response1 = $sg1->client->mail()->send()->post($mail1);
                        $resp1 = $response1->statusCode(); */
                        $guia = $entity->getId();
                        $sucursal = $entity->getGestioncel()->getSucursal();
                        $email = file_get_contents("http://trackers.onlinegeotrack.com.ar/desarrollo/maslogistica/email.php?guia='".$guia ."'&sucursal='".$sucursal."'");
                   
                    }
                    $servicioMaslo = $form['datosenvios']['servicio']->getData();
                    $tiposervicio=  $form['gestioncel']['tiposervicio']->getData();
                    $tipocliente=  $form['gestioncel']['tipocliente']->getData();
                    $ani = $form['gestioncel']['ani']->getData();
                    
                   /* if(($servicioMaslo =='AMBA - Regular' || $servicioMaslo =='INTERIOR - Regular') &&  
                        ($tiposervicio =='5 Dias' || $tiposervicio =='7 Dias' ) && $tipocliente == 'INDIVIDUO'
                    ){
                        //aqui3
                        $smsusuario = "ranval"; //usuario de SMS MASIVOS
                        $smsclave = "mrcce5"; //clave de SMS MASIVOS
                        $smsnumero = $ani;
                        $smstexto = "Tu equipo acaba de ingresar en el circuito de reparacion Movistar";
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($editForm->isSubmitted()) {
            ...
            if(!$user->hasRole('ROLE_ADMIN')){
                if($user->getCliente()->getEmpresa()=='MOVISTAR'){
                    //colo
                    if($entity->getGestionCel()->getEstado() == 'NRCEC - NUEVO REMITO GENERADO EN CEC' ){
                        $estado = $em->getRepository('EstadoBundle:Estado')->findOneBy(array('codigo'=>'NRCEC'));
                        $entity->getGestionCel()->setEstado($estado);
                        $entity->getGestioncel()->setSucursal($user->getSucursal()->getCodSuc());
                        $entity->getGestioncel()->setTrayecto('aGalander');
                        $em->persist($entity);
                        $em->flush();
                        $this->get('session')->getFlashBag()->add('notice', 'Guia Actualizada!');
                    }else{
                    $this->get('session')->getFlashBag()->add('error', 'La guia no se puede modificar por que ya se encuentra en proceso!');
                    }
                }
            }
        ...
        foreach($vars['ids'] as $id_retiro) {
            ...
            if($empresa=='maslogistica'){
                ...
                if($vars['trayecto']=="aMovistar"){
                    $estado = $em->getRepository('EstadoBundle:Estado')->findOneBy(array('codigo'=>'EDETM'));
                ...
                if($vars['trayecto']=="aGalander"){
                    ...
                    $retiro->getGestionCel()->setEstado($estado);
                    //Aqui
                    $tipocliente = $retiro->getGestionCel()->getTipocliente();
                    $tiposervicio = $retiro->getGestionCel()->getTiposervicio();
                    $servicioMaslo = $retiro->getServicio();
                    $ani = $retiro->getGestionCel()->getAni();
                    
                    /*if($tipocliente == 'INDIVIDUO' && ($servicioMaslo == 'AMBA - Regular' || 
                    	$servicioMaslo == 'INTERIOR - Regular')  
                    	&& ($tiposervicio == '5 Dias' || 
                    	$tiposervicio == '7 Dias'  )
                    	){ 
        ...
        try{
            ...
            for($i=1; $i<=2; $i++){
                ...
                if($entity->getGestioncel()->getTrayecto()=='MOVISTAR'){
                    if($i==1){
                        $txt = "1".";"."129".";"."129".";OR;".$manifiestoCarga->getId().";".$fechaDocumento.";".$precinto.";".$cod_clie_dlv_OD.";"."0".";".$nom_Clie_Dlv_OD.";".$dirSender.";".$cpSender.";".$locSender.";1;0;0;1;0;comentarios;".$servicio.";".$remitente.";".$email.";".$isInterior.";"."\r\n";
                    }else{
                        $txt = "1".";"."129".";"."129".";OD;".$manifiestoCarga->getId().";".$fechaDocumento.";".$precinto.";".$cod_clie_dlv_OR.";"."0".";".$nom_Clie_Dlv_OR.";".$dirComprador.";".$cpComprador.";".$locComprador.";1;0;0;1;0;comentarios;".$servicio.";".$apenom.";".$emailComp.";1;"."\r\n";
                    }
                }else{
                    if($i==1){
                        $txt = "1".";"."129".";"."129".";OD;".$manifiestoCarga->getId().";".$fechaDocumento.";".$precinto.";".$cod_clie_dlv_OR.";"."0".";".$nom_Clie_Dlv_OR.";".$dirComprador.";".$cpComprador.";".$locComprador.";1;0;0;1;0;comentarios;".$servicio.";".$apenom.";".$emailComp.";1;"."\r\n";
    ...
    public function getConfirmarBolsinAction(Request $request)
    {
        ...
        //return new Response($attrs["limite"]);
    }
    ...
    public function informarFinalMovistarAction(){
        ...
        $estado_EAC = $em->getRepository('EstadoBundle:Estado')->findBy(array('codigo' => array('EAC')));

        return $this->render('DatosEnviosBundle:DatosEnvios:informar-estados-movistar.html.twig',array(
            'estados' => $estado,
            'estado_eac' => $estado_EAC
        ));
    ...
    public function updateGestionAction(){
        ...
        $em = $this->getDoctrine()->getManager();
        $retiro = $em->getRepository('PresisRetiroBundle:Retiro')->findOneBy(array('id'=>$posted_values['tracking']));
        
        if(!$retiro ){
            return new Response("1");
        }
        $nroserie = $retiro->getGestioncel()->getNroserie();
        $gestion = $retiro->getGestionCel();
        
        if($nroserie != $posted_values['imei']){
            return new Response("2");
        }
        
        if($gestion->getEstado() != 'IH - INGRESO AL HUB' && $user != 'galanderperfil' ){
            return new Response("4");
        }else{
            //VALIDAR SI EL ESTADO ES 167 -> EQUIPO PARA AUDITORIA/CONCILIACION FINALIZAR
            if($posted_values["presis_gestioncelbundle_gestioncel"]["estadointervencion"] != 'PARA AUDITORIA\CONCILIACION'){
                ...
                return new Response("3");
                    ...
                    $estado = $em->getRepository('EstadoBundle:Estado')->findOneBy(array('codigo'=>'EAC'));
                    $gestion->setEstado($estado);
                    $retiro->setEstado($estado);
                    //$gestion->setTrayecto("aMovistar");
                    $em->persist($gestion);
   ...
   //VERIFICA SI UN NRO DE SERIE  / IMEI YA FUE REGISTRADO EN EL DIA
    public function nroSerieFindAction(Request $request){
        ...
        //estado -> equipo entregado al cliente
        $estado = $em->getRepository('EstadoBundle:Estado')->findOneBy(array('codigo'=>'EEC'));

        $existe_creado_misma_fecha = $em->getRepository('GestionCelBundle:GestionCel')->findOneBy(array('nroserie'=>$nroSerie_id,'fechaBase'=>$fecha));
        //$entregado_cliente = $em->getRepository('GestionCelBundle:GestionCel')->findOneBy(array('nroserie'=>$nroSerie_id,'fechaBase'=>$fecha));
        $imei_con_estado_final = $em->createQuery(
            'select gc 
             from GestionCelBundle:GestionCel gc,
             EstadoBundle:Estado e
             where gc.estado = e  
             and gc.nroserie = :nro_serie 
             and gc.estado in (157, 158,159,160,161,162,163,165,166)'
        );


        $imei_con_estado_final->setParameter('nro_serie', $nroSerie_id );

        $result = $imei_con_estado_final->getResult(); 
        

        if($result || $existe_creado_misma_fecha ){
            return new Response(1);
        }
        return new Response(-1);
    }
    ...
    public function findGestionAction(Request $request){
        ...
        $tracking = $request->query->get('tracking');
        $em = $this->getDoctrine()->getManager();
       ...
       /* if (!$gestiones) {
        ...
        $nroserie = $retiro->getGestioncel()->getNroserie();
        $gestion = $retiro->getGestionCel();
        $estado = $gestion->getEstado()->getId();
        if($nroserie != $imei){
        ...
        $response = new Response($serializer->serialize($gestion, "json"));
        $response->headers->set('Content-Type', 'application/json');
    ...
    //ACTUALIZA EL ESTADO DE UNA GESTION EN SU ETAPA FINAL, CUANDO LLEGA A MOVISTAR
     public function updateGestionMovistarAction(){
        ...
        $posted_values = $this->get('request')->request->all();
        $tracking = $posted_values['tracking'];
        $imei = $posted_values['imei'];
        $em = $this->getDoctrine()->getManager();
        $retiro = $em->getRepository('PresisRetiroBundle:Retiro')->findOneBy(array('id'=>$tracking));
        $nroserie = $retiro->getGestioncel()->getNroserie();
        $gestion = $retiro->getGestionCel();
        $estado_id = $posted_values['estado'];
        if($gestion->getEstado() == 'EEC - EQUIPO ENTREGADO AL CLIENTE'){
            return new Response("2");
        }
        if($nroserie != $imei){
            return new Response("1");
                ...
                switch ($estado_id){
                    case 163:
                        if($gestion->getEstado() != 'ERCEC - EQUIPO RECEPCIONADO EN CEC'){
                                return new Response("3");
                                }else{
                                    $tipocliente = $gestion->getTipocliente();
                                    $tiposervicio = $gestion->getTiposervicio();
                                    $ani = $gestion->getAni();
                                    $servicioMaslo = $retiro->getDatosEnvios()->getServicio();
                                    //die("hola" .$servicioMaslo);
                                    ...
                                    $id_estado = $estado->getId();
                                            ...
                                            $estado_id == 163)
                                        {
                                            ...
                                            $smsnumero = $ani;
                                            $smstexto = "Te esperamos en el centro de atencion donde dejaste a reparar tu equipo, para finalizar la gestion. Muchas gracias!";
                                            $smsrespuesta = file_get_contents("http://servicio.smsmasivos.com.ar/enviar_sms.asp?API=1&TOS=". urlencode($smsnumero) ."&TEXTO=". urlencode($smstexto) ."&USUARIO=". urlencode($smsusuario) ."&CLAVE=". urlencode($smsclave) );
                                            $gestion->setEstado($estado);
                                        }
                                    $gestion->setEstado($estado);
                                    ...
                                    $em->persist($tracker);
                                    $em->flush();
                                    return new Response($nombre_estado);
                                }
                        break;
                    case 164:
                        if($gestion->getEstado() != 'TOCEC - TERMINADA OPERATORIO DE ST EN CEC'){
                                return new Response("4"); 
                                }else{
                                    $tipocliente = $gestion->getTipocliente();
                                    $tiposervicio = $gestion->getTiposervicio();
    ...
    public function searchGestionTotusAction(){
        ...
        $estados = $em->getRepository('EstadoBundle:Estado')->findBy(array('codigo'=>'ETHR'));
        ...
        return $this->render('DatosEnviosBundle:DatosEnvios:search-gestioncel-tutos.html.twig', array(
...

    {
     ...
     //BUSCAR GUIA WEB
    public function findGestionCelTotusAction(Request $request)

    {
        ...
        $query->select('r')
            ->from('GestionCelBundle:GestionCel','r')
            //modificado para estado 157 0 158 galander
            ->where('r.estado = 157');
        ...
        $query->orderBy('r.fechaBase','DESC');
